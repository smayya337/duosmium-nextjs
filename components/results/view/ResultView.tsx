...
// @ts-ignore
export default async function ResultView({ data, team }: { data: object, team: number | undefined }) {
  ...
  const tableData: Map<number, { name: string; points: string; place: string; notes: string, medals: number }[]> =
    new Map();
  for (const tm of interpreter.teams) {
